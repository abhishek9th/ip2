...
export const ElectroBorder: React.FC<ElectroBorderProps> = ({
  ...
  const activeEffects = effects && isVisible;
  ...
  const updateFilter = () => {
    ...
    if (strokeLayer.current) {
      strokeLayer.current.style.filter = activeEffects
        ? `url(#${filterId})`
        : "none";
    }
    ...
    requestAnimationFrame(() => {
      [...dxAnimations, ...dyAnimations].forEach((a) => {
        if ("beginElement" in a && typeof a.beginElement === "function") {
          a.beginElement();
        }
      });
    });
  ...
  useEffect(() => updateFilter(), [animationSpeed, distortion, activeEffects]);
    ...
    <div ref={rootRef} className={`relative isolate ${className ?? ""}`} style={style}>
      ...
      <div className="absolute inset-0 pointer-events-none" style={radiusStyle}>
        <div
          ...
          style={borderBase}
        />
        {activeEffects && glow && <div className="absolute inset-0" style={glowLayer1} />}
        {activeEffects && glow && <div className="absolute inset-0" style={glowLayer2} />}
        {activeEffects && aura && <div className="absolute inset-0" style={backgroundAura} />}
      </div>
